      ...
      <div class="max-w-3xl mx-auto py-12 md:py-20">
        ...
        <div class="space-y-8">
          ...
          <template v-for="item in items">
            <div>
              <h4 class="inline-flex font-medium bg-clip-text text-transparent bg-linear-to-r from-green-500 to-purple-200 pb-3">{{ item.area }}</h4>
              <ul class="divide-y divide-slate-800">
                <template v-for="position in item.positions">
                  <li class="flex flex-col md:flex-row flex-wrap justify-between py-5">
                    ...
                    <a class="inline-flex text-sm font-medium text-green-500 group" :href="position.link">{{ position.location }} <span class="tracking-normal group-hover:translate-x-0.5 transition-transform duration-150 ease-in-out ml-1">-&gt;</span></a>
                  </li>
                </template>
              </ul>
            </div>
